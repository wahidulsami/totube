...
//   return (
//     <div className="p-6 sm:p-8 lg:p-12 min-h-screen bg-[#0A0A0A] text-white">
...
//         ) : (
//           <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-pink-500 to-pink-600 flex items-center justify-center text-white font-bold text-xl mr-4">
...
//         <div>{user?.social?.facebook || ""}</div>
//         <div>{user?.social?.twitter || ""}</div>
//         <div>{user?.social?.linkedin || ""}</div>
//         <div>{user?.social?.github || ""}</div>
  ...
  const VideoCard = ({ video, index }) => (
    <div
      className={`group cursor-pointer transition-all duration-300 hover:scale-105 ${
        index === 0 ? "transform hover:scale-110" : ""
      }`}
    >
      ...
      <div className="p-4">
        <h3 className="font-semibold text-white group-hover:text-red-400 transition-colors duration-200 line-clamp-2 mb-2">
          {video.title}
        </h3>
...
<div className="w-full max-w-6xl mx-auto rounded-2xl overflow-hidden ">
  ...
  {/* 2. Channel Info Section (Now below the cover photo) */}
  <div className="p-4 sm:p-6">
    <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6">
      
      {/* Avatar - Negative margin pulls it up to overlap the cover photo slightly */}
      ...
      <div className="flex-1 flex flex-col items-center sm:items-start text-center sm:text-left w-full space-y-4">
        ...
        <div>
          ...
          {user?.fullname && (
            <h2 className="text-lg text-gray-200 mb-1">{user.fullname}</h2>
          )}
 ...
 <div className="relative px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto">
        {/* Channel Header */}
        {/* <div className="flex flex-col lg:flex-row items-start lg:items-end gap-6 -mt-20 lg:-mt-24 pb-8"> */}
          {/* Avatar */}
